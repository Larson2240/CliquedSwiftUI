...
@IBDesignable public class HBSegmentedControl: UIControl {
    ...
    public override func layoutSubviews() {
        ...
        if labels.count > 0 {
            ...
            //Setup gradient image in thumbview
//            let imageName = "square_background"
//            let image = UIImage(named: imageName)
//            let imageView = UIImageView(image: image!)
//            imageView.frame = CGRect(x: 0, y: 0, width: thumbView.frame.size.width, height: thumbView.frame.size.height)
//            imageView.layer.cornerRadius = 15.0
//            imageView.clipsToBounds = true
//            thumbView.addSubview(imageView)
            
            thumbView.frame = label.frame
            thumbView.layer.cornerRadius = 15.0
    ...
    private func displayNewSelectedIndex() {
        ...
        let label = labels[selectedIndex]
        label.textColor = selectedLabelColor
        UIView.animate(withDuration: 0.5, delay: 0.0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.8, animations: { [weak self] in
